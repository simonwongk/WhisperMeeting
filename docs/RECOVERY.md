...
| `meetings.json` or `vocabulary.json` is damaged | The app opens the previous readable backup, which may be one save behind, and leaves the damaged primary exactly as it is. The library opens read-only and nothing is written until recovery is resolved. | The backup, the damaged primary, and every recording folder. |
